...
// 1. Read the newly parsed products
const allProducts = JSON.parse(fs.readFileSync('d:/New-Impact-Care/src/data/new_parsed_products.json', 'utf8'));

// The user wants to add them in batches of 6. Let's take the first 6.
const firstBatch = allProducts.slice(0, 6);

console.log("First Batch:");
firstBatch.forEach(p => console.log(p.id + ". " + p.title + " (Img: " + p.imgSource + ")"));

// 2. Copy the first 6 images to public/assets/uploads/products/
// We need to make sure the target directory exists
const targetImgDir = 'd:/New-Impact-Care/public/assets/uploads/products/';
if (!fs.existsSync(targetImgDir)) {
    fs.mkdirSync(targetImgDir, { recursive: true });
}

firstBatch.forEach(p => {
    const sourceImg = path.join('d:/New-Impact-Care/docx_extract/word/media/', p.imgSource);
    const ext = path.extname(p.imgSource);
    const targetImgName = p.slug + ext;
    const targetImg = path.join(targetImgDir, targetImgName);
    
    // Copy file
    if (fs.existsSync(sourceImg)) {
        fs.copyFileSync(sourceImg, targetImg);
        p.finalImage = '/assets/uploads/products/' + targetImgName;
    } else {
        console.warn("Missing image: " + sourceImg);
        p.finalImage = '/assets/uploads/products/tablet_mockup.png';
    }
});

// 3. Prepare the new array format for products.json
const jsonProducts = firstBatch.map(p => ({
    id: p.id.toString(),
    slug: p.slug,
    name: p.title,
    category: p.category,
    ...
    packDetails: p.packDetail,
    description: p.description,
    image: p.finalImage
}));

fs.writeFileSync('d:/New-Impact-Care/src/data/products.json', JSON.stringify(jsonProducts, null, 2));
console.log("Updated src/data/products.json");

// 4. Update src/lib/data/products.ts
...
firstBatch.forEach((p, idx) => {
    tsContent += `  {
    "id": ${p.id},
    "title": ${JSON.stringify(p.title)},
    "slug": ${JSON.stringify(p.slug)},
    "category": ${JSON.stringify(p.category)},
    "dosageForm": ${JSON.stringify(p.dosageForm)},
    "composition": ${JSON.stringify(p.composition)},
    "packDetail": ${JSON.stringify(p.packDetail)},
    "img": ${JSON.stringify(p.finalImage)},
    "description": ${JSON.stringify(p.description)}
  }${idx < firstBatch.length - 1 ? ',' : ''}\n`;
});
tsContent += `];\n`;

fs.writeFileSync('d:/New-Impact-Care/src/lib/data/products.ts', tsContent);
console.log("Updated src/lib/data/products.ts");

// 5. We also need to fix `rebuild_products.js` and `generate_products.js` to NOT use hardcoded old products,
// but rather read from our `products.json` or `products.ts`.
// For this first batch, we will rewrite `rebuild_products.js` to ONLY generate HTML for these 6 items.
...
fs.writeFileSync('d:/New-Impact-Care/apply_batch.js', rebuildScript);
console.log("Created apply_batch.js to rebuild products.html");

// 6. Write a script to generate individual product pages
...
files.forEach(file => {
    if (file.startsWith('product__') && file !== 'product__acofan-tablet.html') {
...
firstBatch.forEach(product => {
    ...
    newContent = newContent.replace(
        ...
        '<h1 class="elementor-heading-title elementor-size-default">' + product.name + '</h1>'
    );
    ...
    newContent = newContent.replace(
        ...
        '<span class="elementor-icon-list-text"><strong>Pack Details:</strong> ' + product.packDetails + '</span>'
    );
    ...
    // Replace Image
    newContent = newContent.replace(
        /\\/assets\\/uploads\\/products\\/acofan_tablet_1779861526962\\.png/g,
        product.image
    );
...
// Optionally delete the template so it doesn't show up in routes (or keep it as backup)
// For now, we will just rename it so it doesn't conflict
if (fs.existsSync(templatePath)) {
    fs.copyFileSync(templatePath, path.join(__dirname, 'acofan_template_backup.html'));
    fs.unlinkSync(templatePath); // Removing old hardcoded page
}

console.log('Successfully created new product HTML files and cleared old ones!');
`;

fs.writeFileSync('d:/New-Impact-Care/generate_batch_pages.js', generateScript);
console.log("Created generate_batch_pages.js to generate detail pages.");
